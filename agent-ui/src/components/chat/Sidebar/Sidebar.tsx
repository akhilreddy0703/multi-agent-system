...
const Sidebar = ({
  ...
  // Apply redirect params from backend login: #access_token=...&endpoint=...&team=...
  useEffect(() => {
    ...
    if (endpoint) {
      const url = decodeURIComponent(endpoint).replace(/\/$/, '').trim()
      if (url) {
    ...
    if (team) {
      const teamIdValue = decodeURIComponent(team).trim()
      if (teamIdValue) {
        setMode('team')
  ...
  }, [hydrated, setAuthToken, setSelectedEndpoint, setMode, setTeamId, setAgentId])
